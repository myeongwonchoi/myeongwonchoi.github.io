...
SELECT DISTINCT POI_ID, FNAME, CNAME, CP_ID, ADDR +' '+ CONCAT(ADDR, ' ', PRIMARY_BUN, '-', SECONDARY_BUN) AS [구주소],
CONCAT_WS('-', NULLIF(TELE_A, ''), NULLIF(TELE_B, ''), NULLIF(TELE_C, '')) AS [전화번호]
...
SELECT  POI_ID, FNAME, CNAME, CP_ID, ADDR +' '+ CONCAT(ADDR, ' ', PRIMARY_BUN, '-', SECONDARY_BUN) AS [구주소],
CONCAT_WS('-', NULLIF(TELE_A, ''), NULLIF(TELE_B, ''), NULLIF(TELE_C, '')) AS [전화번호]
