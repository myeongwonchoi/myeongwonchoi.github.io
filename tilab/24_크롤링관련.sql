...
FROM vPOI_I_COMMON_ALL_GRS AS A WITH (NOLOCK)
INNER JOIN POI_MAIN_ETC..[22y_2150_INDEX_220401] AS B ON LEFT(A.TILE_ID, 4) = B.MAP_ID
...
WHERE (ISNULL(LTRIM(RTRIM(A.FNAME)), '') = '')
  AND EXISTS (SELECT 1 FROM POI_MAIN_ETC..[22y_2150_INDEX_220401] B WHERE LEFT(A.TILE_ID, 4) = B.MAP_ID AND B.관할업체 = '티아이랩')
ORDER BY A.POI_ID;
